...
export async function POST(
  ...
  const { data: storeData } = await admin
    .from('stores')
    .select('user_id, shop_domain, judgeme_api_token, google_connection_mode')
    .eq('id', review.store_id)
    .maybeSingle()

  if (!storeData) return NextResponse.json({ error: 'Not found' }, { status: 404 })

  const store = storeData as Pick<StoreRow, 'user_id' | 'shop_domain' | 'judgeme_api_token' | 'google_connection_mode'>
